    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="p-8 bg-white border border-slate-100 rounded-[2.5rem] shadow-sm">
           <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-6">Environment Mode</label>
           <div className="grid grid-cols-2 gap-4">
              <button 
                onClick={() => updateTheme({ mode: 'light' })}
                className={`p-6 rounded-3xl border-2 transition-all flex flex-col items-center gap-3 ${theme.mode === 'light' ? 'border-blue-500 bg-blue-50/50' : 'border-slate-100 bg-slate-50'}`}
              >
                <Sun size={24} className={theme.mode === 'light' ? 'text-blue-600' : 'text-slate-400'} />
                <span className="text-xs font-bold uppercase tracking-widest text-slate-900">Light Mode</span>
              </button>
              <button 
                onClick={() => updateTheme({ mode: 'dark' })}
                className={`p-6 rounded-3xl border-2 transition-all flex flex-col items-center gap-3 ${theme.mode === 'dark' ? 'border-blue-500 bg-slate-800' : 'border-slate-100 bg-slate-50'}`}
              >
                <Moon size={24} className={theme.mode === 'dark' ? 'text-blue-400' : 'text-slate-400'} />
                <span className={`text-xs font-bold uppercase tracking-widest ${theme.mode === 'dark' ? 'text-white' : 'text-slate-900'}`}>Dark Mode</span>
              </button>
           </div>
        </div>
