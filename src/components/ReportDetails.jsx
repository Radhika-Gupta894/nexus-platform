...
import { Shield, ShieldOff, Lock, User, MapPin, AlertTriangle, CheckCircle2, Clock, X, MessageCircle, Info } from "lucide-react";
...
export default function ReportDetails({ report, user, onClose }) {
  ...
  React.useEffect(() => {
    if (activeTab === "chat" && report) {
      chatService.getOrCreateChatRoom(report, user);
    }
  ...
  return (
    <div className="fixed inset-0 z-[2000] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-5xl h-[85vh] overflow-hidden animate-in slide-in-from-bottom-8 duration-500 border border-slate-100 flex flex-col">
        
        {/* Header Section */}
        <div className={`p-6 flex items-center justify-between ${report.privacy === 'Private' ? 'bg-red-50' : 'bg-[var(--color-nexus-light)]'}`}>
           <div className="flex items-center gap-4">
              <div className={`p-3 rounded-2xl ${report.privacy === 'Private' ? 'bg-red-500 text-white shadow-[0_0_20px_rgba(239,68,68,0.3)]' : 'bg-[var(--color-nexus-primary)] text-white shadow-[0_0_20px_rgba(59,130,246,0.3)]'}`}>
                 {report.privacy === 'Private' ? <ShieldOff size={24} /> : <Shield size={24} />}
              </div>
              <div>
                <h2 className="text-xl font-black text-slate-900 uppercase tracking-tighter leading-none">{report.type || "Intelligence Report"}</h2>
                <div className="flex items-center gap-3 mt-1.5">
                   <span className={`text-[10px] font-black uppercase tracking-widest ${report.privacy === 'Private' ? 'text-red-600' : 'text-[var(--color-nexus-primary)]'}`}>
           ...
           <div className="flex bg-white/50 p-1.5 rounded-2xl border border-slate-200">
              <button 
                ...
                className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 transition-all ${activeTab === 'intel' ? 'bg-white text-[var(--color-nexus-primary)] shadow-sm' : 'text-slate-400'}`}
              >
                <Info size={14} /> Intelligence
              </button>
              <button 
                onClick={() => setActiveTab("chat")}
                className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 transition-all ${activeTab === 'chat' ? 'bg-white text-[var(--color-nexus-primary)] shadow-sm' : 'text-slate-400'}`}
              >
                <MessageCircle size={14} /> Comms
              </button>
           </div>
          ...
          {activeTab === "intel" ? (
            <div className="p-10 grid grid-cols-1 lg:grid-cols-2 gap-10">
               ...
               <div className="space-y-8">
                  ...
                  {report.proofImage && (
                    <div className="space-y-2">
                       <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Field Proof (Photo)</label>
                       <img src={report.proofImage} alt="Proof" className="w-full h-48 object-cover rounded-[2rem] border border-slate-100 shadow-sm" />
                    </div>
                  )}
               </div>
               ...
               <div className="space-y-8">
                  ...
                  {!isAdmin && !isAssigned && isPublic && !isCreator && (
                    <div className="p-6 bg-amber-50 rounded-3xl border border-amber-100">
                       <p className="text-amber-800 text-xs font-medium leading-relaxed">
                         <span className="font-black uppercase tracking-widest block mb-1">Public Access Mode</span>
                         You are viewing this intelligence in read-only mode. Tactical operations and private communication are restricted to authorized personnel.
                       </p>
                    </div>
                  )}
               </div>
               ...
               <div className="h-full flex flex-col">
                  <div className="flex-1 overflow-hidden">
                    <ChatBox 
                      user={user} 
                      reportId={report.id} 
                      title={`Mission Comms: ${report.type}`}
                    />
                  </div>
               </div>
            </div>
        ...
        <div className="px-10 py-6 bg-slate-50 border-t border-slate-100 flex items-center justify-between">
           <div className="flex items-center gap-4">
              {isAdmin && (
                <span className="px-4 py-2 bg-slate-900 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg">Admin Control</span>
              )}
           </div>
           <button 
             onClick={onClose}
