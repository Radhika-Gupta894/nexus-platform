...
import { n as createLayerComponent } from "./generic-DbSOLpn2.js";
//#region node_modules/simpleheat/simpleheat.js
var require_simpleheat = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	simpleheat.prototype = {
		...
		draw: function(minOpacity) {
			...
			for (var i = 0, len = this._data.length, p; i < len; i++) {
				p = this._data[i];
				ctx.globalAlpha = Math.max(p[2] / this._max, minOpacity === void 0 ? .05 : minOpacity);
		...
		_createCanvas: function() {
			...
			else return new this._canvas.constructor();
		}
	};
}));
//#endregion
//#region node_modules/react-leaflet-heatmap-layer-v3/lib/Heatmap.js
var import_leaflet_src = /* @__PURE__ */ __toESM(require_leaflet_src());
var import_simpleheat = /* @__PURE__ */ __toESM(require_simpleheat());
...
function computeAggregate(agg, intensity, aggregateType = "sum") {
	...
	if (!agg.data[type]) if (type === "min") agg.data[type] = Number.MAX_SAFE_INTEGER;
	else if (type === "max") agg.data[type] = Number.MIN_SAFE_INTEGER;
	else if ([
		"stdev",
		"stdevp",
		"variance",
		...
		if (!agg.data.mean) agg.data.mean = 0;
		if (!agg.data.sigma) agg.data.sigma = 0;
		agg.data[type] = 0;
	} else agg.data[type] = 0;
	const res = (fns[type] || fns.sum)(agg.data[type], agg.seen, intensity);
	if ([
		"mean",
		"count",
		"sum"
	].includes(type)) agg.data[type] += res;
	else agg.data[type] = res;
	return agg.data[type];
}
var Heatmap = class extends import_leaflet_src.Layer {
	constructor(options) {
		super(options);
		this.options = import_leaflet_src.Util.setOptions(this, options);
	}
	get _heatmap() {
		if (!this.__heatmap) {
	...
	onAdd(map) {
		const zoomClass = `leaflet-zoom-${map.options.zoomAnimation && import_leaflet_src.Browser.any3d ? "animated" : "hide"}`;
		...
		this.reset();
		if (this.options.fitBoundsOnLoad) this.fitBounds();
		this.updateSimpleHeat(this.getSimpleHeatOptions());
		return this;
	}
	onRemove() {
		const pane = this.getPane();
		if (pane.contains(this._el)) pane.removeChild(this._el);
		return this;
	}
	getEvents() {
		return {
	...
	_animateZoom(e) {
		...
		import_leaflet_src.DomUtil.setTransform(this._el, offset, scale);
	}
	fitBounds() {
		...
		if (shouldIgnoreLocation(ne) || shouldIgnoreLocation(sw)) return;
		this._map.fitBounds(import_leaflet_src.latLngBounds(import_leaflet_src.latLng(sw), import_leaflet_src.latLng(ne)));
	}
	resize() {
		if (!this._map) return;
	...
	getMinOpacity() {
		var _a;
		return (_a = this.options.minOpacity) !== null && _a !== void 0 ? _a : .01;
	}
	getOpacity() {
		var _a;
	...
	*/
	updateHeatmapRadius(radius, blur) {
		if (isNumber(radius)) this._heatmap.radius(radius, blur);
	}
	/**
	* Update the heatmap's gradient
	*/
	updateHeatmapGradient(gradient) {
		if (gradient) this._heatmap.gradient(gradient);
	}
	/**
	* Update the heatmap's maximum
	*/
	updateHeatmapMax(maximum) {
		if (isNumber(maximum)) this._heatmap.max(maximum);
	}
	redraw() {
		if (!this._map) return;
		...
		const roundResults = (results) => results.reduce((result, row) => filterUndefined(row).map((cell) => [
			Math.round(cell[0]),
			Math.round(cell[1]),
			cell[2]
		]).concat(result), []);
		...
		const accumulateInGrid = (points, leafletMap, bounds, aggregateType) => points.reduce((grid, point) => {
			...
			if (isInvalidLatLngArray(latLng)) return grid;
			const p = leafletMap.latLngToContainerPoint(latLng);
			if (!inBounds(p, bounds)) return grid;
		...
		const getBounds = () => new import_leaflet_src.Bounds(import_leaflet_src.point([-r, -r]), size.add([r, r]));
		...
		if (this.options.useLocalExtrema) this.updateHeatmapMax(totalMax);
		try {
			this._heatmap.draw(this.getMinOpacity());
		} catch (DOMException) {}
		this._frame = null;
		if (this.options.onStatsUpdate && this.options.points && this.options.points.length > 0) this.options.onStatsUpdate({
	...
	reset() {
		...
		import_leaflet_src.DomUtil.setPosition(this._el, topLeft);
		this.resize();
		if (this._heatmap && !this._frame) this._frame = import_leaflet_src.Util.requestAnimFrame(this.redraw, this);
		this.redraw();
	}
};
//#endregion
//#region node_modules/react-leaflet-heatmap-layer-v3/lib/HeatmapLayer.js
var HeatmapLayer = createLayerComponent(function createHeatmapLayer(props, context) {
	return {
		instance: new Heatmap(props),
		context
	};
	...
	import_leaflet_src.Util.setOptions(instance, {
		latitudeExtractor,
		longitudeExtractor,
		intensityExtractor,
		points,
...
//#endregion
var SimpleHeat = import_simpleheat.default;
export { Heatmap, HeatmapLayer, SimpleHeat, computeAggregate };
